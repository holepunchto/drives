...
function onpeertick(diff, drive, state = {}) {
  ...
  function onpeers() {
    ...
    for (const [id, info] of peers) {
      ...
      output.push(
        '- ' +
          ...
          ' + ' +
          crayon.yellow(
            (info.blobs.peer?.remoteContiguousLength || 0) + '/' + (drive.blobs?.core.length || 0)
          ) +
          ' blks'
      )
    ...
    if (peers.size - count > 0) {
      output.push('... and ' + crayon.yellow(peers.size - count) + ' more connections')
    }
