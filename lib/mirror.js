...
module.exports = async function mirror(store, swarm, src, dst, options = {}) {
  ...
  const filter = createMirrorFilter(await loadIgnorePatterns(source))
  ...
  const doMirror = async function () {
    const m = source.mirror(destination, { prefix: '/', filter })
  ...
  for await (const { diff } of watcher) {
    for (const { key } of diff) {
      ...
      const diffObj = {
        ...
        op: isDelete ? 'remove' : isNew ? 'add' : 'change'
      }
      ...
      if (isDelete) {
        ...
        await pipeline(
          ...
          destination.createWriteStream(key, {
...
const ignore = /\/(\.DS_Store)(\/|$)/i

function createMirrorFilter(patterns) {
  const shouldIgnore = patterns.length ? picomatch(patterns, { dot: true }) : null

  return function (key) {
    if (ignore.test(key)) return false

    if (!shouldIgnore) return true
    return !shouldIgnore(key.slice(1)) // strip leading “/” from drive key before matching
  }
}

async function loadIgnorePatterns(source) {
  const buffer = await source.get('/.drivesignore')
  if (!buffer) return []
...
function formatCount(count) {
  return (
    crayon.green('+' + count.add) +
    ' ' +
    crayon.red('-' + count.remove) +
    ' ' +
    crayon.yellow('~' + count.change)
  )
}
...
async function same(srcEntry, dstEntry, srcDrive, dstDrive) {
  ...
  return streamEquals(srcDrive.createReadStream(srcEntry), dstDrive.createReadStream(dstEntry))
}
...
function getDrive(arg, corestore, options) {
